...
def calculateRecharge(dfs,date_range):
    ...
    costRockImagerTime = 2.50 / 60.0 #per min
    ...
    adjPayroll_lst = [i - j for i,j in zip(lst_payroll,monthlyRechargeTotal['Facility Fee'].values)]

    monthlyRechargeTotal['Payroll Cost'] = [max(i,j) for i, j in zip(
        monthlyRechargeTotal['Facility Fee'].values, adjPayroll_lst)]
