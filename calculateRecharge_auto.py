...
gc = pyg.authorize(service_file='/Users/liam_msg/Documents/MSG_automate_recharge/msg-Recharge-24378e029f2d.json')
...
def calculateRecharge(dfs,date_range):
    ...
    q = df_screenOrders.groupby([pd.Grouper(freq="M"),'Group']).apply(lambda x: x.head(
        len(x.index)))[wantedSOCol].loc[start_date:end_date]


    # EDIT: fixed code on jan 14 2020 
    ...
    itemizedSOMonthByPI = itemizedSOMonthByPI[itemizedSOMonthByPI["Screens Total Cost"]!='']
...
if __name__ == '__main__':
    ...
    start_date, end_date = '', ''
    end_d= date.today().replace(day=1)
    ...
    print(df_RockImager_1[df_RockImager_1.Group == 'nan'])
